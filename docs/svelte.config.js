	...
	kit: {
		...
		prerender: {
			handleHttpError: ({ path, message }) => {
				// Cross-domain links (landing lives on a separate domain)
				if (
					path.startsWith('/try-app') ||
					path.startsWith('/support') ||
					path.startsWith('/about') ||
					path.startsWith('/thesis') ||
					path.startsWith('/trust-and-safety')
				) {
					console.warn(`Ignoring prerender 404 for cross-domain link: ${path}`);
					return;
				}
